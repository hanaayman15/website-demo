...
function AddClient() {
  ...
  }, [isEditMode, navigate, redirectClientId]);

  if (loading) {
    return <main className="react-page-wrap">Loading client basic info...</main>;
  }

  return (
    <main className="react-page-wrap react-grid" style={{ gap: '1rem', maxWidth: 1000 }}>
      <section className="react-panel">
        <h1 style={{ marginTop: 0, marginBottom: '0.4rem' }}>{isEditMode ? 'Edit Client' : 'Add New Client'}</h1>
        <p className="react-muted" style={{ margin: 0 }}>{isEditMode ? 'Update Basic Information' : 'Basic Information only'}</p>
      </section>
      ...
      <section className="react-panel">
        ...
        <form className="react-grid" onSubmit={saveBasicInfo}>
          <div className="react-grid react-grid-2">
            ...
            <label>
              ...
              <input
                ...
                minLength={6}
                placeholder={isEditMode ? 'Leave blank when editing' : 'Minimum 6 characters'}
                required={!isEditMode}
                disabled={isEditMode}
              />
            </label>
          ...
          <div style={{ display: 'flex', gap: '0.6rem', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
            <button className="react-btn react-btn-ghost" type="button" onClick={() => navigate('/clients')}>
              Cancel
            </button>
            {!isEditMode ? (
